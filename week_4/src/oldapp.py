...
#Imports orders, products and couriers from csv files to working lists of dictionaries
...
def main_menu_sel():
    while True:
        ...
        if user_input == "0":#EXIT APP
            ...
            #Write the working products and couriers list to the existing csv file then closes the app

            with open("data\\products.csv", "w") as products_raw:
                writer=csv.DictWriter(products_raw, fieldnames=["name", "price"])
                writer.writeheader()
                writer.writerows(products)

            with open("data\\couriers.csv", "w") as couriers_raw:
                writer=csv.DictWriter(couriers_raw, fieldnames=["company", "name", "phone"])
                writer.writeheader()
        ...
        else:#INVALID ENTRY, SELECT FROM THE MENU
            print("Invalid selection, please select from list (0, 1, etc).")
            continue


#PRODUCTS MENU SELECTION

def product_menu_sel(): 
    while True:
        product_menu()
        user_input = (input("Please select an option:\n"))
...
#COURIER MENU SELECTION

def courier_menu_sel():
    while True:
        courier_menu()
